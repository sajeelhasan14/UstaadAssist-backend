...
PATCH  /courses/:id                      edit course settings (attendance_threshold, dates, class_days)
...
GET    /courses/:id/assessments          list them
POST   /assessments/:id/marks            enter marks
GET    /assessments/:id/marks            marks already entered, for the entry screen
...
GET    /courses/:id/weightage             component percentages
...
| M5 | *(removed from scope)* | Quiz question tagging and topic-wise analysis were cut on 22 Sep 2026. See `docs/ROADMAP.md`. A quiz is simply an assessment of type `quiz`, marked as a single total like any other. |
...
**Topic-wise analysis is not in this version.** It required the teacher to tag every quiz question with a topic by hand, which contradicts the minimum-input rule in Section 1. It returns when a vision model can read a question paper and assign topics automatically, with the teacher only reviewing. The reasoning is written up in `docs/ROADMAP.md`.

What still makes this more than a management system is **Section 3 — the planner**. Slot generation, topic allocation, assessment date validation, replanning that freezes the past, and computed deficit resolution are the graded core, and none of them are affected by this cut.
